...
public class Font {
	...
	public static void drawDkNorthumbriaString(String text, float x, float y, int size, Color color, FontType type) {
		...
		if (type == FontType.RENDERED) {
			if (sizeIndex < 3)
				throw new RuntimeException("There is no rendered font in that size. Please try font sizes 40, 45, 50");
    ...
    public static void drawTriniganFgString(String text, float x, float y, int size, Color color, FontType type) {
        ...
        if (type == FontType.RENDERED) {
            if (sizeIndex < 4)
                throw new RuntimeException("There is no rendered font in that size. Please try font sizes 24, 30");
            else fontColor = FontColor.NO_COLOR;
        }
    ...
    private static int getDKNIndexFromFontSize(int size) {
        ...
        if (size == 45) return 4;
        if (size == 50) return 5;
        throw new RuntimeException(size + " is an invalid DK Northumbria font size. Please choose from the following: 25, 30, 35, 40, 45, 50");
    }
    ...
    private static int getTFGIndexFromFontSize(int size) {
        ...
        if (size == 24) return 4;
        if (size == 30) return 5;
        throw new RuntimeException(size + " is an invalid Trinigan FG font size. Please choose from the following: 10, 12, 16, 20, 24, 30");
    }
    ...
    private static FontColor getFontColorFromColor(Color color) {
        ...
        throw new RuntimeException("The color " + color + "is not supported. Please try red, white, or black");
    }
	/**
