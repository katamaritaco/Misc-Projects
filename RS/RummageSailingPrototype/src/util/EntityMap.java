...
public class EntityMap implements Serializable {
	private int GRID = 8;//this size of one grid space
	...
	private void hashEntity(Entity entity){
		Rectangle hitbox = entity.getBounds();
		int gx = hitbox.getX()/GRID;
		int gy = hitbox.getY()/GRID;
		if(hitbox.getX() < 0)gx--;
		if(hitbox.getY() < 0)gy--;
		int gwidth =  (hitbox.getWidth()/GRID) + 1;
		int gheight = (hitbox.getHeight()/GRID) + 1;
...
//		);
	...
	public ArrayList<Entity> get(Rectangle rectangle){
		int gx = rectangle.getX()/GRID;
		int gy = rectangle.getY()/GRID;

		if(rectangle.getX() < 0)gx--;
		if(rectangle.getY() < 0)gy--;

		int gwidth =  (rectangle.getWidth()/GRID) + 1;
		int gheight = (rectangle.getHeight()/GRID) + 1;
		ArrayList<Entity> returning = new ArrayList<Entity>();
		boolean useRect = gwidth*gheight < entities.size();
		if(!useRect){
			for (Entity entity : entities){
				if(entity.getBounds().intersects(rectangle)){
	...
	public ArrayList<Entity> get(Point point){
		Rectangle rect = new Rectangle(point.getX()-GRID/2, point.getY()-GRID/2, GRID, GRID);
		return get(rect);
	}
