...
class ProgramNode extends ASTnode {
    ...
    public void nameAnalysis() {
        ...
        Sym testForMain = symTab.lookupGlobal("main");
    
        if (testForMain == null || !(testForMain instanceof FnSym))
        {
            ErrMsg.fatal(0,0, "No main function");
        }

        //symTab should be at the lowest/global scope
        //just iterate through that and set all to global
        /*Iterator < Hashmap <String, Sym> > itr = symTab.localIterator(); 
        ...
        while (itr.hasNext())
        {
            ...
            pairs.getValue().setIsGlobal();

        }*/

        //technically, there should only be one map in the list by now
        //but I get the last one just to be safe
        symTab.setGlobals();
    ...
    public void codeGen()
    {
        //System.out.println("mf doom smiles upon us --- IMPLEMENT CODE GEN!!!");
    	
    	myDeclList.codeGen();
    	//Codegen.generate("yoloswag");
        
        for (Map.Entry<String,String> entry : Codegen.strLitTable.entrySet()) {
//            System.out.print("key,val: ");
//            System.out.println(entry.getKey() + "," + entry.getValue());
        	
        	Codegen.generate(".data");
        	
        	//might need to be wrapped in quotes
        	Codegen.generateLabeled(entry.getValue(), ".asciiz", "" , entry.getKey());
...
class VarDeclNode extends DeclNode {
    ...
    public void codeGen(){
    	Codegen.generate(".data\n.align 2\n");
    	Codegen.genLabel("_" + myId);
    	Codegen.generate(" .space 4\n");//4 bytes because we're limited to bools/ints
    }
...
class FnDeclNode extends DeclNode {
    ...
    public void codeGen()
    {
    	if (myId.name().equals("main"))
    	{
    		...
    		Codegen.generateLabeled("main", "", "Main decl");
    	}
    	else
    	{
    		Codegen.generate(".text");
    		Codegen.generateLabeled(myId.name(), "", "Function decl");
    	}
    	
    	//function entry
    	Codegen.generateIndexed("sw", Codegen.RA, Codegen.SP, 0, "Push return addr")
    	Codegen.generate("subu", Codegen.SP, Codegen.SP, 4);
    	
    	Codegen.generateIndexed("sw", Codegen.FP, Codegen.SP, 0, "Push control link");
    	Codegen.generate("subu", Codegen.SP, Codegen.SP, 4);
    	
    	//TODO get the size of the params
    	//4 bytes for ints and bools, the only passable thing
    	int paramSize = myFormalsList.length() * 4; //Nice dude XXX
    	
    	//Plus 8 for control link and ret addr
    	Codegen.generate("addu", Codegen.FP, Codegen.SP, paramSize + 8);
...
class WriteStmtNode extends StmtNode {
	...
	//if it's a string do it one way, otherwise it's a bool or int
	//if bool or int, just print int
    ...
    public void typeCheck(Type retType) {
        ...
        else  {//This is an error that will never get to code gen
        	typeFlag = -1;
        	ErrMsg.fatal(0,0, "SHOULD NEVER SEE THIS - CODEGEN IN WRITESTMT :'[");
        }
...
//        generate("syscall");
        ...
        Codegen.generate("li", Codegen.V0, typeFlag);
...
class StringLitNode extends ExpNode {
    ...
    public void codeGen(){
    	...
    	//if (!Codegen.strLitTable.containsKey(myStrVal))
    	
    	Codegen.strLitTable.put(myStrVal, Codegen.nextLabel());
    }
...
class EqualsNode extends EqualityExpNode {
    ...
    public void codeGen()
    {
        ...
        {
            //code gen exp1.val.equals(exp2.val)
            //TODO should just be a TRUE or FALSE
        }
    }
}
